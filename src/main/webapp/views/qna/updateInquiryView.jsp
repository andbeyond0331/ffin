		...
		$(function () {
			$("a[href='#' ]").click(function () {
				...
				if(inquiryTypeChk&&inquiryTitleChk&&inquiryContentChk){
					$("#updateInquiry").attr("method", "POST").attr("action","/qna/updateInquiry").attr("enctype","multipart/form-data").submit();
				}
...
<section class="client_section layout_padding">
	<form class="row justify-content-center" id="updateInquiry" >
						...
						<div class="col-md-8 data-input-box">
							...
							<div class="data-input-box">
								...
								<select class="form-select form-control" id="inquiryType" name="inquiryType" aria-label="Default select example">
									<c:choose>
										<c:when test="${inquiry.inquiryType eq 1}"><option selected style="color: grey">사이트 이용</option></c:when>
										<c:when test="${inquiry.inquiryType eq 2}"><option selected style="color: grey">푸드트럭</option></c:when>
										<c:when test="${inquiry.inquiryType eq 3}"><option selected style="color: grey">주문</option></c:when>
										<c:when test="${inquiry.inquiryType eq 4}"><option selected style="color: grey">기타</option></c:when>
									</c:choose>
							...
							<div class="data-input-box">
								<label for="inquiryTitle" class="form-label label-name" style="margin-top: 30px;">제목</label>
								<input type="text" class="form-control" id="inquiryTitle" name="inquiryTitle" value="${inquiry.inquiryTitle}">
								<span class="inquiryTitleChk" style="display: none;">제목을 입력해주세요.</span>
							</div>
							<div class="data-input-box">
								<label for="inquiryContent">내용</label>
								<textarea class="form-control label-name" id="inquiryContent" name="inquiryContent" style="resize: none; height: 100px">${inquiry.inquiryContent}</textarea>
							...
							<div class="data-input-box">
								...
								<div class="text-center">
									<c:if test="${inquiry.inquiryFile != null}">
										<img src="../../resources/image/${inquiry.inquiryFile}" class="rounded" alt="...">
									</c:if>
								</div>
